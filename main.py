...
class AgentGUI(ctk.CTk):
    def __init__(self):
        ...
        self.btn_kill = ctk.CTkButton(self, text="⏻ Принудительно выключить", command=self.force_quit,
                                      fg_color="#222", hover_color="#550000", font=("Arial", 11))
        self.btn_kill.pack(pady=(0, 10))

        # Автозапуск после /restart
    ...
    def setup_settings_tab(self):
        ...
        def _label(parent, text):
            return ctk.CTkLabel(parent, text=text, font=F_BOLD)

        def _unit(parent, text):
            return ctk.CTkLabel(parent, text=text, font=F)

        # Ключи с подсказками и ссылками на получение
        KEY_LINKS = {
        ...
        def _make_link_label(parent, display_name, url):
            """Кликабельная метка с hover-эффектом — открывает ссылку в браузере."""
            frame = tk.Frame(parent, bg="#2b2b2b", cursor="hand2")
            ...
            def _on_enter(_):
                text_lbl.configure(fg="#89c4f4")
                icon.configure(fg="#89c4f4")
            def _on_leave(_):
                text_lbl.configure(fg="#5a9fd4")
                icon.configure(fg="#5a9fd4")
            def _open(_):
                webbrowser.open(url)
        ...
        for key, (display_name, url) in KEY_LINKS.items():
            ...
            if is_secret:
                btn = ctk.CTkButton(tab, text="👁", width=40, font=F,
                                    fg_color="#555", hover_color="#777",
                                    command=lambda ent=e: self.toggle_visibility(ent))
                btn.grid(row=current_row, column=2, padx=10, pady=5)
            current_row += 1

        _label(tab, "Main Model:").grid(row=current_row, column=0, padx=10, pady=5, sticky="w")
        self.e_model_main = ctk.CTkEntry(tab, font=F)
        ...
        _label(tab, "Expert Model:").grid(row=current_row, column=0, padx=10, pady=5, sticky="w")
        self.e_model_expert = ctk.CTkEntry(tab, font=F)
        ...
        self.e_work_dir = ctk.CTkEntry(tab, font=F,
                                        placeholder_text="(по умолчанию — рядом с программой)")
        self.e_work_dir.insert(0, config.get_config("work_dir") or "")
        self.e_work_dir.grid(row=current_row, column=1, padx=(10, 0), pady=5, sticky="we")
        self._bind_entry(self.e_work_dir)

        def _browse_work_dir():
            from tkinter import filedialog
            d = filedialog.askdirectory(title="Выберите рабочую папку")
            if d:
                self.e_work_dir.delete(0, "end")
                self.e_work_dir.insert(0, d)

        ctk.CTkButton(tab, text="📂", width=40, font=F,
                      fg_color="#555", hover_color="#777",
                      command=_browse_work_dir).grid(row=current_row, column=2, padx=10, pady=5)
        current_row += 1

        # --- Фоновая активность: интервал + чекбокс автозапуска ---
        ...
        self.e_bg_h = ctk.CTkEntry(frame_bg, width=52, font=F); self.e_bg_h.insert(0, str(total_sec // 3600)); self.e_bg_h.pack(side="left", padx=(0, 2)); self._bind_entry(self.e_bg_h)
        _unit(frame_bg, "ч").pack(side="left", padx=(0, 6))
        self.e_bg_m = ctk.CTkEntry(frame_bg, width=52, font=F); self.e_bg_m.insert(0, str((total_sec % 3600) // 60)); self.e_bg_m.pack(side="left", padx=(0, 2)); self._bind_entry(self.e_bg_m)
        _unit(frame_bg, "м").pack(side="left", padx=(0, 6))
        self.e_bg_s = ctk.CTkEntry(frame_bg, width=52, font=F); self.e_bg_s.insert(0, str(total_sec % 60)); self.e_bg_s.pack(side="left", padx=(0, 2)); self._bind_entry(self.e_bg_s)
        _unit(frame_bg, "с").pack(side="left", padx=(0, 10))

        self._bg_autostart_var = tk.BooleanVar(value=bool(config.get_config("bg_autostart")))
        ctk.CTkCheckBox(
            frame_bg, text="запускать при старте",
            variable=self._bg_autostart_var,
            font=F, onvalue=True, offvalue=False,
        ).pack(side="left")
        current_row += 1

        # --- Макс. итераций ---
        _label(tab, "Макс. итераций:").grid(row=current_row, column=0, padx=10, pady=5, sticky="w")
        self.e_max_iter = ctk.CTkEntry(tab, font=F)
        ...
        # --- Размер краткосрочной памяти ---
        _label(tab, "История сообщений:").grid(row=current_row, column=0, padx=10, pady=5, sticky="w")
        frame_hist = ctk.CTkFrame(tab, fg_color="transparent")
        frame_hist.grid(row=current_row, column=1, padx=(10, 0), pady=5, sticky="w")
        self.e_history_limit = ctk.CTkEntry(frame_hist, width=70, font=F)
        self.e_history_limit.insert(0, str(config.get_config("history_limit") or 40))
        self.e_history_limit.pack(side="left", padx=(0, 6))
        self._bind_entry(self.e_history_limit)
        _unit(frame_hist, "последних сообщений").pack(side="left")
        current_row += 1

        # --- Уровень логов ---
        _label(tab, "Уровень логов:").grid(row=current_row, column=0, padx=10, pady=5, sticky="w")
        self.c_log = ctk.CTkComboBox(tab, values=["DEBUG", "INFO", "WARNING", "ERROR"], font=F)
    ...
    def _bind_entry(entry: ctk.CTkEntry):
        """Навешивает на CTkEntry корректные хоткеи для любой раскладки + undo/redo."""
        inner = entry._entry  # внутренний tk.Entry

        # Ручной undo-стек (tk.Entry не поддерживает edit_undo как tk.Text)
        undo_stack: list[tuple[str, int]] = []   # (текст, позиция курсора)
        redo_stack: list[tuple[str, int]] = []
        _last_saved = [""]
        ...
        def _ctrl_bs(e=None):
            try:
                ...
                val = inner.get()[:pos]
                stripped = val.rstrip()
                word_start = stripped.rfind(" ") + 1
                inner.delete(word_start, pos)
            except Exception: pass
            return "break"

        def _on_key(e):
            # Снимаем снапшот на каждое изменение для undo
            inner.after(10, _snapshot)

        def _on_ctrl(e):
            # Keycode-независимые хоткеи — работают при любой раскладке
            k = e.keycode
            if k == 65: return _select_all()
            if k == 67: return _copy()
            if k == 86: return _paste()
            if k == 88: return _cut()
            if k == 90:
                return _redo() if (e.state & 0x1) else _undo()

        inner.bind("<KeyRelease>",        _on_key)
        inner.bind("<Control-KeyPress>",  _on_ctrl)
        inner.bind("<Command-KeyPress>",  _on_ctrl)
        inner.bind("<Control-BackSpace>", _ctrl_bs)
        inner.bind("<Command-BackSpace>", _ctrl_bs)
    ...
    def setup_chat_tab(self):
        ...
        # Кнопки пакуются ПЕРВЫМИ справа — иначе pack не оставит им места
        def _make_icon_btn(parent, text, command, tooltip_text):
            btn = ctk.CTkButton(parent, text=text, width=40, height=40,
                                font=("Segoe UI Emoji", 18),
                                fg_color="transparent", hover_color="#3a3a3a",
                                corner_radius=8, command=command)
            btn.pack(side="left", padx=2)
            tooltip = tk.Label(self, text=tooltip_text, bg="#1a1a1a", fg="white",
                               font=("Segoe UI", 10), padx=6, pady=3, relief="flat")
            def _show(e): tooltip.place(x=btn.winfo_rootx() - self.winfo_rootx(),
                                        y=btn.winfo_rooty() - self.winfo_rooty() - 30)
            def _hide(e): tooltip.place_forget()
        ...
        def _show_commands_menu():
            ...
            for cmd in telegram_app.BOT_COMMANDS:
                label = f"/{cmd.command}  —  {cmd.description}"
                def _run_cmd(c=cmd.command):
                    self.msg_entry.delete("1.0", "end")
                    self.msg_entry.insert("1.0", f"/{c}")
                    self.msg_entry.focus()
                    # Отправляем сразу, как в Telegram
                    self.after(50, self.send_gui_msg)
                menu.add_command(label=label, command=_run_cmd)
            btn = self._cmd_menu_btn
            try:
                menu.tk_popup(btn.winfo_rootx(), btn.winfo_rooty() - menu.winfo_reqheight() - 4)
            finally:
                menu.grab_release()
        ...
        _make_icon_btn(icon_frame, "➤", self.send_gui_msg,  "Отправить (Enter)")

        # Поле ввода пакуется после кнопок — занимает оставшееся место
        self.msg_entry = ctk.CTkTextbox(frame, height=40, wrap="word", font=("Segoe UI", 13),
        ...
        def _resize_entry(event=None):
            """Подстраивает высоту поля ввода под содержимое (макс. 5 строк)."""
            lines = int(self.msg_entry._textbox.index("end-1c").split(".")[0])
            new_h = max(40, min(lines * 22 + 10, 120))
            self.msg_entry.configure(height=new_h)

        def _on_key(event):
            self.after(10, _resize_entry)

        def _entry_send(event):
        ...
        def _entry_ctrl(event):
            c = event.char
            k = event.keycode
            if k == 65 or c == '\x01':   # Ctrl+A — выделить всё
                self.msg_entry._textbox.tag_add("sel", "1.0", "end")
                return "break"
            if k == 67 or c == '\x03':   # Ctrl+C — копировать
                try:
                    self.clipboard_clear()
                    self.clipboard_append(self.msg_entry.selection_get())
                except Exception: pass
                return "break"
            if k == 86 or c == '\x16':   # Ctrl+V — вставить
                try:
                    self.msg_entry.insert("insert", self.clipboard_get())
                    self.after(10, _resize_entry)
                except Exception: pass
                return "break"
            if k == 88 or c == '\x18':   # Ctrl+X — вырезать
                try:
                    sel = self.msg_entry.selection_get()
                    self.clipboard_clear()
                    self.clipboard_append(sel)
                    self.msg_entry._textbox.delete("sel.first", "sel.last")
                except Exception: pass
                return "break"
            if k == 90:                  # Ctrl+Z / Ctrl+Shift+Z
                if event.state & 0x1:
                    try: self.msg_entry._textbox.edit_redo()
                    except Exception: pass
                else:
                    try: self.msg_entry._textbox.edit_undo()
                    except Exception: pass
                return "break"

        def _entry_ctrl_bs(event):
            """Ctrl+Backspace — удалить слово влево."""
            try:
                idx = self.msg_entry._textbox.index("insert")
                line, col = map(int, idx.split("."))
                text_before = self.msg_entry._textbox.get(f"{line}.0", idx)
                # Ищем начало предыдущего слова
                stripped = text_before.rstrip()
                word_start_col = stripped.rfind(" ") + 1
                self.msg_entry._textbox.delete(f"{line}.{word_start_col}", idx)
            except Exception: pass
            return "break"

        self.msg_entry.bind("<Return>",            _entry_send)
        self.msg_entry.bind("<KeyPress>",          _on_key)
        # Вешаем хоткеи напрямую на внутренний tk.Text — keycode-независимо, любая раскладка
        self.msg_entry._textbox.bind("<Control-KeyPress>",  _entry_ctrl)
        self.msg_entry._textbox.bind("<Command-KeyPress>",  _entry_ctrl)
        self.msg_entry._textbox.bind("<Control-BackSpace>", _entry_ctrl_bs)
        self.msg_entry._textbox.bind("<Command-BackSpace>",  _entry_ctrl_bs)
    ...
    def attach_copy_bindings(self, textbox):
        ...
        menu = tk.Menu(self, tearoff=0, bg="#2b2b2b", fg="white",
                       activebackground="#1f538d", activeforeground="white", borderwidth=0)
    ...
    # ── Единые константы шрифта для всего чата ──────────────────────────────
    FONT_MAIN   = ("Segoe UI", 13)          # основной текст
    FONT_BOLD   = ("Segoe UI", 13, "bold")
    FONT_ITALIC = ("Segoe UI", 13, "italic")
    FONT_CODE   = ("Consolas", 12)          # моноширинный для кода
    FONT_META   = ("Segoe UI", 11, "bold")  # «Агент» / «Вы»
    COLOR_CODE  = "#e6db74"
    COLOR_META  = "#888888"
    COLOR_STATUS_ICON = "#aaaaaa"           # эмодзи статуса чуть приглушены

    def _configure_tags(self, tb):
        """Регистрирует базовые теги оформления в текстовом виджете."""
        inner = tb._textbox
    ...
    def _hide_scrollbar(self, tb):
        """Прячет вертикальный скроллбар CTkTextbox (имя атрибута зависит от версии)."""
    ...
    def insert_markdown(self, tb, text):
        """Рендерит Markdown (**bold**, *italic*, `code`, ```block```, [текст](url)) в textbox."""
        import webbrowser
        self._configure_tags(tb)
        inner = tb._textbox

        def _strip_inline(s):
            """Убирает **..** / *..*  / _.._  / __..__  из строки — для label ссылок."""
            s = re.sub(r'\*\*(.+?)\*\*', r'\1', s)
        ...
        for seg in splitter.split(text):
            ...
            if m_link:
                ...
                inner.insert("end", seg[3:-3].strip('\n') + '\n', "code")
            elif seg.startswith('`') and seg.endswith('`') and len(seg) > 2:
                inner.insert("end", seg[1:-1], "code")
            elif seg.startswith('**') and seg.endswith('**') and len(seg) > 4:
                inner.insert("end", seg[2:-2], "bold")
            elif seg.startswith('__') and seg.endswith('__') and len(seg) > 4:
                inner.insert("end", seg[2:-2], "bold")
            elif seg.startswith('*') and seg.endswith('*') and len(seg) > 2:
                inner.insert("end", seg[1:-1], "italic")
            elif seg.startswith('_') and seg.endswith('_') and len(seg) > 2:
                inner.insert("end", seg[1:-1], "italic")
            else:
                inner.insert("end", seg, "normal")


    def _calc_tb_height(self, text: str, chars_per_line: int = 72) -> int:
        """Считает высоту текстового виджета в пикселях по количеству строк."""
        line_px = 21        # высота строки при Segoe UI 13
        pad_px  = 20        # внутренние отступы CTkTextbox сверху+снизу
        lines = 0
        for line in text.split('\n'):
            # +1 за саму строку, чтобы не было 0 строк для пустых
            lines += max(1, (max(0, len(line) - 1) // chars_per_line) + 1)
        # Минимум 1 строка, небольшой запас чтобы не появлялся скролл
    ...
    def append_chat(self, sender, text, replace_last=False, close_bubble=False):
        ...
        msg_frame = None  # будет присвоен при создании нового пузыря
        ...
        def _fill_tb(tb, content, is_final):
            ...
            # Широкий предварительный размер — чтобы tk успел сделать layout
            tb.configure(height=self._calc_tb_height(content))
            self._hide_scrollbar(tb)

            def _fit_to_content():
                """Точная подгонка высоты через dlineinfo — читает реальные пиксели."""
                try:
                    inner = tb._textbox
                    ...
                    if info is None:
                        return False  # tk ещё не отрисовал — повторим
                    # y + height последней строки = реальная высота содержимого
                    content_px = info[1] + info[3]
                    try:
                        pad_y = int(inner.cget("pady") or 2) * 2 + 8
                    except Exception:
                        pad_y = 12
                    tb.configure(height=content_px + pad_y + 24)  # +2-3 строки запаса
                    self._hide_scrollbar(tb)
                    return True
                except Exception:
                    self._hide_scrollbar(tb)
        ...
        if not replace_last or getattr(self, 'current_bot_textbox', None) is None or not isinstance(text, str):
            ...
            bubble = ctk.CTkFrame(
                msg_frame,
                fg_color="#2b2b2b" if is_agent else "#1a4a7a",
                corner_radius=12,
            )
            bubble.pack(
                side="left" if is_agent else "right",
                fill="x", expand=True,
                padx=(0 if is_agent else 60, 60 if is_agent else 0),
            )
            if is_agent:
                self.current_bot_frame = msg_frame

            ctk.CTkLabel(bubble, text=sender,
                         font=self.FONT_META, text_color=self.COLOR_META
    ...
    def _make_ctk_image(self, img_pil: Image.Image, max_size: tuple = (400, 400)) -> ctk.CTkImage:
        """Конвертирует PIL Image в CTkImage — корректно масштабируется на HiDPI."""
    ...
    def _load_pil_from_data_uri(self, data_uri: str):
        """Декодирует data:image URI → PIL Image (или None при ошибке)."""
    ...
    def _open_image_viewer(self, img_pil: Image.Image):
        """Модальное окно с полноразмерным изображением и кнопкой сохранения."""
        ...
        ctk.CTkButton(btn_frame, text="💾 Сохранить", command=_save,
                      width=130, fg_color="#555", hover_color="#777").pack(side="left", padx=6)
        ctk.CTkButton(btn_frame, text="✕ Закрыть", command=viewer.destroy,
                      width=100, fg_color="#3a3a3a", hover_color="#550000").pack(side="left", padx=6)

    def send_gui_msg(self):
        msg = self.msg_entry.get("1.0", "end").strip()
        if not msg or not self.is_running: return
        self.msg_entry.delete("1.0", "end")
        self.msg_entry.configure(height=40)

        # Обработка slash-команд в GUI (как в Telegram)
        if msg.startswith("/"):
            ...
            if cmd == "clear":
                ...
                async def _do_screenshot():
                    ...
                    def _finish():
                        ...
                        if result.startswith("Ошибка"):
                            self.append_chat("Система", result, close_bubble=True)
                        else:
                            self.append_chat(
                                "Система",
                ...
                self._dispatch_to_agent(prompt)
                return
            elif cmd == "restart":
                self.append_chat("Система", "🔄 Перезапуск недоступен в GUI-режиме. Используйте кнопку остановки и запуска.", close_bubble=True)
                return
            elif cmd == "shutdown":
                self.force_quit()
                return
    ...
    def _dispatch_to_agent(self, content):
        """Отправляет сообщение агенту — через очередь если сессия активна, иначе новый run."""
    ...
    def send_file_gui(self):
        ...
        if mime and mime.startswith("image/"):
            ...
            # Конвертируем в PDF через DynamicPDF API асинхронно
            async def _convert_and_send():
                ...
                if pdf_path:
                    ...
                    # DynamicPDF не настроен или недоступен — передаём путь
                    content = [{"type": "text", "text": (
                        f"Пользователь прислал файл: {filename}\n"
                        f"Полный путь: {filepath}\n"
                        f"Конвертация в PDF недоступна (проверь DYNAMICPDF_API_KEY).\n"
            ...
            return  # dispatch произойдёт внутри корутины
            ...
            content = [
                {"type": "text", "text": f"Пользователь прислал файл `{filename}`:\n\n```\n{text_content[:8000]}\n```"},
            ]
            self._dispatch_to_agent(content)
    ...
    def setup_logs_tab(self):
        ...
        self._hide_scrollbar(self.log_box)
    ...
    def force_quit(self):
        """Немедленно убивает весь процесс, не дожидаясь завершения потоков и задач."""
        logging.warning("⏻ Принудительное выключение...")
        os._exit(0)
    ...
    def run_async_bot(self):
        ...
        token = config.get_config("TELEGRAM_TOKEN").strip()
        bot = None
        if token:
            try:
                bot = Bot(token=token)
            except Exception as e:
                logging.warning(f"⚠️ Не удалось создать Telegram-бота: {e}. Агент запущен без Telegram.")
        else:
            logging.info("ℹ️ TELEGRAM_TOKEN не указан — агент работает только в режиме GUI.")

        bg_autostart = config.get_config("bg_autostart")

        async def bg_worker():
            interval = config.get_config("bg_interval") or 28800
            if bg_autostart:
                logging.info("🔄 Фоновая активность при старте включена — первый запуск через 5с.")
                await asyncio.sleep(5)
            else:
                logging.info(f"💤 Фоновая активность при старте отключена — первое пробуждение через {interval // 3600}ч {(interval % 3600) // 60}м.")
        ...
        async def main_task():
            ...
            if bot is not None:
                ...
                logging.info("✅ Агент запущен (только GUI, без Telegram).")
                while True:
