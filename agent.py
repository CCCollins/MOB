...
class SendMessageDraft(TelegramMethod[bool]):
    """
    Прямой вызов метода sendMessageDraft из новых версий Bot API.
    Создан как отдельный класс, чтобы код работал даже на старых версиях aiogram.
    """
    __returning__ = bool
...
SYSTEM_PROMPT = """Ты — автономный, проактивный ИИ-агент, управляющий устройством (Windows/Linux).
Твои возможности:
1. Выполнение команд терминала (полный доступ).
2. Поиск в интернете.
3. Чтение/запись файлов.
4. Долгая память.
5. Инициация общения с пользователем через send_telegram_message.

САМОУЛУЧШЕНИЕ: Ты можешь создавать новые скрипты (в папке ./custom_tools/) для автоматизации рутины. Если тебе не хватает инструмента, напиши код на Python, сохрани в файл и запускай через терминал. Улучшай свой код, если он выдает ошибки.
ФОНОВАЯ АКТИВНОСТЬ: Периодически ты будешь просыпаться без команды пользователя. В эти моменты проверяй систему (нагрузку, логи, обновления), вспоминай свои долгосрочные цели из памяти и выполняй их. Если делать нечего, отвечай 'IDLE'. Если нашел что-то важное или выполнил задачу в фоне, сообщи пользователю!"""
...
async def convert_to_pdf_dynamicpdf(file_path: str, original_filename: str) -> str | None:
    if not DYNAMICPDF_API_KEY:
        log.warning("Ключ DYNAMICPDF_API_KEY не найден.")
        return None
...
async def execute_terminal(command: str) -> str:
    try:
        if platform.system() == "Windows":
            # Принудительно заставляем PowerShell выдавать ответ в UTF-8, чтобы избежать ошибок декодирования
        ...
        # Заменяем битые символы на '?', чтобы не сломать JSON OpenRouter
...
# --- ЦИКЛ АГЕНТА (С НАСТОЯЩИМ СТРИМИНГОМ DRAFTS) ---
...
async def run_agent(user_id: int, user_message, is_background=False, tg_message: types.Message = None):
    ...
    # Используем ID сообщения пользователя в качестве ID черновика (рекомендация Telegram API)
    ...
    async def update_draft(text: str, parse_mode: Optional[str] = None):
        """Прячет текст в черновик над клавиатурой пользователя. Не засоряет историю!"""
        ...
        try:
            ...
            # Игнорируем ошибки (например, если клиент пользователя еще не поддерживает API 9.3)
            pass
    ...
    while iterations < max_iterations:
        ...
        try:
            ...
            async for chunk in response_stream:
                ...
                if delta.content:
                    ...
                    # Обновляем черновик каждые 0.3 секунды. Для Drafts лимиты Telegram намного мягче!
                    # parse_mode=None нужен, чтобы сырой Markdown от нейросети не сломал HTML-парсер
            ...
            await update_draft("") # Очищаем черновик перед сообщением об ошибке
            if tg_message:
                await tg_message.answer(f"❌ <b>Ошибка сервера нейросети:</b>\n<code>{e}</code>", parse_mode="HTML")
            chat_histories[user_id].pop()
            return None

        tool_calls = list(tool_calls_dict.values())

        # Если инструментов нет - это финальный ответ пользователю.
        if not tool_calls:
            ...
            chat_histories[user_id].append(assistant_msg)
            return full_text
        ...
        # Если ИИ написал текст перед вызовом инструмента — отправляем его отдельным сообщением
        if full_text and tg_message:
            try:
                await bot.send_message(chat_id, full_text, parse_mode="Markdown")
            except Exception:
        ...
        for tc in tool_calls:
            ...
            # Показываем красивый HTML-статус выполнения программы В ЧЕРНОВИКЕ
            ...
            chat_histories[user_id].append({"role": "tool", "tool_call_id": tc["id"], "name": func_name, "content": str(result)})
            
            await update_draft("⏳ <i>Анализирую результат...</i>", parse_mode="HTML")

    await update_draft("") # Очищаем черновик, если достигли лимита
    if tg_message: await tg_message.answer("⚠️ Агент достиг лимита итераций (задач). Остановлен для предотвращения зацикливания.")
    return None

# --- ФОНОВАЯ АКТИВНОСТЬ (ПРОАКТИВНОСТЬ) ---
...
async def handle_files(message: types.Message):
    ...
    # Мини-хелпер для черновиков на этапе загрузки
    async def update_draft(text: str):
        try:
            await bot(SendMessageDraft(
                chat_id=message.chat.id, draft_id=message.message_id, 
                text=text, parse_mode="HTML", message_thread_id=message.message_thread_id
            ))
        except: pass
    ...
    if ext in ['docx', 'doc', 'xlsx', 'xls']:
        await update_draft("⏳ <i>Конвертирую документ в PDF для ИИ...</i>")
        ...
        if pdf_path:
            ...
            content[0]["text"] += f"\n\n[СИСТЕМА]: Сбой конвертации. Исходник сохранен: {tmp_path}. Используй терминал для его анализа."
        ...
        content[0]["text"] += f"\n\n[СИСТЕМА]: Файл сохранен локально по пути: {tmp_path}. Используй терминал."

    await update_draft("") # Удаляем черновик статуса файла и отдаем управление ИИ
    await run_agent(message.from_user.id, content, tg_message=message)

async def main():
    log.info(f"Агент запущен на ОС: {platform.system()}")
